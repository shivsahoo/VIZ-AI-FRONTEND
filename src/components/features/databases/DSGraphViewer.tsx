...
import { Database, Link2, KeyRound, TableProperties, ZoomIn, ZoomOut, Maximize2 } from "lucide-react";
...
export function DSGraphViewer({ graph }: DSGraphViewerProps) {
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-[1fr_300px] gap-4 h-[70vh]">
      <div className="rounded-xl border border-border bg-[#091225] p-3 overflow-hidden relative">
        <div className="absolute top-3 right-3 z-20 flex items-center gap-2">
          <button
            className="h-8 w-8 rounded-md bg-slate-900/80 border border-slate-700 text-slate-200 flex items-center justify-center"
        ...
        <div
          className={`w-full h-full min-h-[520px] rounded-lg ${isPanning ? "cursor-grabbing" : "cursor-grab"}`}
          onWheel={handleWheel}
            ...
            return (
              <g key={edge.id}>
                <path
                  ...
                  markerEnd="url(#arrow)"
                />
                <text x={lx} y={ly} textAnchor="middle" fontSize="10" fill="#8CDFFF">
                  {edge.label}
                </text>
              </g>
            );
      ...
      <div className="rounded-xl border border-border bg-card p-4 space-y-4 overflow-auto">
        ...
        {selectedNode ? (
          <div className="space-y-3">
            <div className="rounded-lg bg-muted/30 border border-border p-3">
              <p className="text-sm text-foreground truncate">{selectedNode.label}</p>
            ...
            <div className="max-h-[220px] overflow-auto rounded-md border border-border">
              {selectedNode.columns.map((col) => (
                <div key={`${selectedNode.id}-${col.name}`} className="flex items-center justify-between px-3 py-2 border-b border-border/60 last:border-b-0">
                  ...
                  <span className="text-muted-foreground text-[11px] ml-2">{col.type}</span>
                </div>
              ))}
            </div>
          </div>
