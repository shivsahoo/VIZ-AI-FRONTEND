...
import { Plus, ChevronDown, LayoutDashboard, Clock, Loader2, MessageSquare } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
interface PreviewChart {
  id?: string;
  name: string;
  type: 'line' | 'bar' | 'pie' | 'area' | 'none' | string; 
  description?: string;
  query?: string;
  reasoning?: string;
  interaction?: string;
...
export function ChartPreviewDialog({ isOpen, onClose, chart, dashboards = [], projectId, onAddToDashboard, onSaveAsDraft, isExistingChart = false, chartStatus: _chartStatus }: ChartPreviewDialogProps) {
  ...
  React.useEffect(() => {
    if (!isOpen) {
      ...
      const cleanupTimeout = setTimeout(() => {
        const dropdownPortals = document.querySelectorAll('[data-slot="dropdown-menu-portal"]');
        dropdownPortals.forEach(portal => {
          try { if (portal.parentNode) portal.parentNode.removeChild(portal); } catch (e) {}
        });
        const radixPortals = document.querySelectorAll('[data-radix-portal]');
        radixPortals.forEach(portal => {
          const isDropdownPortal = portal.querySelector('[data-slot="dropdown-menu-content"]');
  ...
  React.useEffect(() => {
    ...
    if (chart.data && chart.dataKeys && chart.xAxisKey) {
      ...
      setChartDataConfig({ data: processedData, dataKeys: chart.dataKeys, xAxisKey: chart.xAxisKey });
    } else {
      setChartDataConfig(getDefaultChartDataConfig());
    }
  ...
  React.useEffect(() => {
    ...
    if (chart.data && chart.data.length > 0 && !chart.isLoadingData && !chart.dataError) return;
    ...
    const executeQuery = async () => {
      ...
      try {
        ...
        if (!cancelled) setIsExecutingQuery(false);
      }
    };
  ...
  const missingConfigMessage = !hasQuery
    ...
    : !hasConnection
      ? "Please select a valid database connection before previewing this chart."
      : undefined;
  const noDataReturned = !isExecutingQuery && !chartDataError && chartDataConfig.data.length === 0;
  const cachedAtDisplay = chartDataMetadata?.cachedAt && !Number.isNaN(Date.parse(chartDataMetadata.cachedAt))
      ? new Date(chartDataMetadata.cachedAt).toLocaleString()
      : chartDataMetadata?.cachedAt ?? null;

  const resolveAxisFields = () => {
    const resolvedXAxis = chart?.xAxisKey || chart?.xAxisField || chart?.spec?.x_axis || chartDataMetadata?.xAxis || chartDataConfig.xAxisKey;
    const resolvedYAxis = chart?.dataKeys?.primary || chart?.yAxisField || chart?.spec?.y_axis || chartDataMetadata?.yAxis || chartDataConfig.dataKeys.primary;
    return { xAxis: resolvedXAxis ?? null, yAxis: resolvedYAxis ?? null };
  };
  ...
  const handleOpenChange = (open: boolean) => {
    if (!open) {
      ...
      if (pendingDashboardCallbackRef.current !== null) {
        ...
        const callbackTimeout = setTimeout(() => {
          const dropdownPortals = document.querySelectorAll('[data-slot="dropdown-menu-portal"]');
          dropdownPortals.forEach(portal => { try { if (portal.parentNode) portal.parentNode.removeChild(portal); } catch (e) {} });
        ...
        <DialogHeader className="px-3 sm:px-4 md:px-5 pt-3 sm:pt-4 md:pt-5 pb-2 sm:pb-3 flex-shrink-0 pr-12 sm:pr-14 relative">
          <div className="flex items-start justify-between gap-2 sm:gap-3">
            <div className="flex-1 min-w-0">
              ...
              <DialogDescription className="text-xs break-words line-clamp-2">
                {isConversational ? "Agent Response" : (chart.description || chart.reasoning || "Review the chart details before saving or adding it to a dashboard.")}
              </DialogDescription>
                ...
                <div className="absolute inset-0 flex items-center justify-center p-6">
                  <div className="flex flex-col items-center max-w-md bg-background/60 p-6 rounded-xl border border-border shadow-sm">
                    ...
                    <p className="text-sm md:text-base text-foreground leading-relaxed text-center font-medium">
                      {chart.interaction || chart.reasoning || chart.description || "Hello! How can I help?"}
                    </p>
                  </div>
                </div>
              ) : missingConfigMessage ? (
